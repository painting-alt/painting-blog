...
function calcDayFromNow(t){
  let startDay=new Date();
  let endDay=new Date(t);
  let timeDifference = Math.abs(startDay.getTime() - endDay.getTime());
  let differentDays = Math.floor(timeDifference / (1000 * 3600 * 24));
  return differentDays;
}

let memoryDay=[1,3,7,20];
...
class ArticleController {
  ...
  static async getList(ctx) {
    ...
    if (validator) {
     ...
     // console.log("data ",data.rows[0].dataValues.a=2)


      data.rows.forEach(_data=>{
        let change_obj=_data.dataValues;
